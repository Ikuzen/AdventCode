{
  let grid = [];
  let currentPosxy = [[0, 0],[0,0]];

  let field = document.getElementById("field") as HTMLCanvasElement;
  let drawer = field.getContext('2d');
  // drawer.
  let direction = {
    'R': [0, 1],
    'L': [0, -1],
  ...
  function draw(path,color,num) {
    ...
    switch (thisDirection) {
      ...
      case 'L':
        for (let i = 0; i < multiplier; i++) {
          currentPosxy[num][1] += direction[thisDirection][1];
          drawer.fillRect(currentPosxy[num][0]/5, currentPosxy[num][1]/5, 1, 1);
        }
        break;
      case 'R':
  ...
  for (let i = 0; i < inputx.length; i++) {
    draw(inputx[i],'red',0)
    draw(inputy[i],'blue',1)
    console.log(currentPosxy)
  }
}